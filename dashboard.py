...
def simulate_traffic():
    ...
    for i in range(samples):
        # Decide if this sample is an attack (30% probability)
        is_attack = random.random() > 0.7
        
        # Generate IP (more realistic for demo)
        source_ip = f"192.168.1.{random.randint(2, 254)}"
        
        # Generate features with correct dimensions
        features = generate_sample_features(is_attack)
    ...
    return jsonify({"success": True, "count": samples, "detections": results})
...
def recent_detections():
    ...
    return jsonify(sorted(detections[-10:], key=lambda x: x['timestamp'], reverse=True))
